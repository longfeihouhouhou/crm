...
var server1 = http.createServer(function (request, response) {
    ...
    if (pathname === '/addInfo') {
        ...
        request.on('end', function () {
            ...
            fs.readFile('./json/custom.json', 'utf-8', function (err, data) {
                data = JSON.parse(data);
                console.log(requestStr,'requestStr数据')
                console.log(data,'data数据')
                data.push(requestStr);

                console.log(data,'data数据')
                fs.writeFile('./json/custom.json', JSON.stringify(data),'utf-8', function (err) {
                    if (err) {
                        result = {
                            code: 1,
                            msg: 'error'
